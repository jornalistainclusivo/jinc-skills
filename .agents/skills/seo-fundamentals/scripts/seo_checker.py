...
SKIP_DIRS = {
    ...
    '__tests__', 'spec', 'docs', 'documentation', 'examples', '.strapi'
}
...
def check_page(file_path: Path) -> dict:
    ...
    # Detect if this is a layout/template file or Next.js layout (has Head component or metadata export)
    is_layout = bool(re.search(r'<Head[>\s]|<head[>\s]', content))
    has_next_metadata = 'export const metadata' in content or 'export async function generateMetadata' in content
    
    # 1. Title tag
    has_title = '<title' in content.lower() or 'title=' in content or has_next_metadata
    if not has_title and (is_layout or has_next_metadata):
        issues.append("Missing <title> tag")
    
    # 2. Meta description
    has_description = 'name="description"' in content.lower() or 'name=\'description\'' in content.lower() or has_next_metadata
    if not has_description and (is_layout or has_next_metadata):
        issues.append("Missing meta description")
    
    # 3. Open Graph tags
    has_og = 'og:' in content or 'property="og:' in content.lower() or has_next_metadata
    if not has_og and (is_layout or has_next_metadata):
