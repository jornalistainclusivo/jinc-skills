...
class UXAuditor:
    ...
    def audit_file(self, filepath: str) -> None:
        ...
        filename = filepath # changed to full path for debugging
        ...
        # --- 1. PSYCHOLOGY LAWS ---
        # Hick's Law - Check nav elements only to avoid counting all page links
    ...
    def audit_directory(self, directory: str) -> None:
        ...
        for root, dirs, files in os.walk(directory):
            dirs[:] = [d for d in dirs if d not in {'node_modules', '.git', 'dist', 'build', '.next', '.strapi'}]
            for file in files:
