...
| 3        | **Still ambiguous → ask.** If no rule breaks the tie, do NOT guess. Surface the options through the Socratic Gate (Phase 0). The number of questions must depend on missing decision-critical information. Avoid unnecessary questioning when project type is already clear. | "an app for my shop" → ask: web, mobile, or desktop?                                                       |
